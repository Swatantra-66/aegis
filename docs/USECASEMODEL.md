...
- **Experiment No:** 05
- **Experiment Title:** UML Use Case Modeling & Detailed Use Case Descriptions
- **Student Name:** Swatantra Yadav
- **Roll Number:** 2400290130175
- **Branch / Section:** Information Technology (IT-C)
...
- **Modeling Tool:** StarUML (v6.x) / UML 2.0 Specification
...
3. **Cryptographic Tamper-Evidence:** Append-only audit logs linked via cryptographic SHA-256 hash chains ($H_n = \text{SHA-256}(H_{n-1} \parallel \text{Payload})$).

---

## 4. ACTOR IDENTIFICATION & TAXONOMY
...
## 5. USE CASE RELATIONSHIP MATRIX
...
📁 [**`docs/uml/Aegis_IAM_UseCase_Model.mdj`**](file:///c:/Users/HP/Desktop/IAM-Portal/docs/uml/Aegis_IAM_UseCase_Model.mdj)

![IAM Portal Use Case Diagram](file:///c:/Users/HP/Desktop/IAM-Portal/docs/uml/assets/IAM%20Portal%20Use%20Case%20Diagram.jpg)

---

## 7. DETAILED USE CASE NARRATIVES / SPECIFICATIONS
...
| **Typical Course of Events** | **1.** System presents MFA challenge screen.<br>**2.** Actor provides current 6-digit TOTP code.<br>**3.** System decrypts stored AES-256-GCM secret and validates code against current time window ($\pm 1$ step tolerance).<br>**4.** Code validated; authentication completes. |
...
| **Typical Course of Events** | **1.** User initiates logout.<br>**2.** System extracts `jti` and remaining lifetime ($TTL = \text{exp} - \text{now}$).<br>**3.** System executes `SETEX bl:<jti> <TTL> 1` in Redis.<br>**4.** System deletes refresh token family from Redis and clears client cookies. |
...
| **Secondary Actor** | PostgreSQL DB |
| **Use Case Type** | **`«include»` Sub-Routine Use Case** |
| **Description** | Traverses all audit log records from genesis ($H_0$), recomputing $H_n = \text{SHA-256}(H_{n-1} \parallel \text{Timestamp} \parallel \text{Actor} \parallel \text{Action})$ to mathematically verify that zero database rows have been modified, deleted, or injected. |
