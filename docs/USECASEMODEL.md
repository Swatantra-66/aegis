...
| `Enroll / Manage MFA` | `Authenticate & Sign In` | Dependency | **`«extend»`** | Zero-Trust enrollment branch conditionally intercepting promoted administrators without MFA, issuing scoped `mfa:enroll_only` token. |
...
| **Use Case Type** | Core Concrete Use Case (includes UC-02a, extended by UC-02b, extended by UC-05 Flow 5a) |
| **Description** | Validates identity credentials, initiates a cryptographically signed JWT token pair (Short-lived Access Token + RTR Refresh Token), and creates an active session. When authenticating an administrator promoted without prior MFA enrollment, enforces the Zero-Trust policy: denies standard session token issuance and returns an ephemeral scoped MFA Enrollment Token (`scope: 'mfa:enroll_only'`) to mandate MFA configuration before administrative elevation. |
| **Pre-Conditions** | User possesses an active, unblocked account in PostgreSQL. |
| **Post-Conditions** | **1. Standard Flow:** User receives signed RS256/HS256 Access Token (15-min TTL) and Refresh Token (7-day TTL). Refresh token hash and family lineage are persisted in PostgreSQL DB.<br>**2. Zero-Trust Promotion Exception:** If user was promoted to an administrative tier (`admin` or `super_admin`) without prior MFA enrollment, **standard access and refresh tokens are NOT issued**. Instead, user receives a short-lived (10-min) scoped MFA Enrollment Token (`scope: 'mfa:enroll_only'`), and the account state transitions to mandatory MFA setup (UC-05 Flow 5a).<br>**3. Audit Telemetry:** Successful login event (`AUTH_LOGIN`) or enrollment token issuance (`MFA_ENROLLMENT_TOKEN_ISSUED`) is chained into the audit log. |
| **Typical Course of Events (Normal Flow)** | **1. Actor Action:** User submits username/email and password.<br>**2. System Response:** System executes **`«include»` UC-02a: Validate Password (Argon2id)**.<br>**3. System Response:** System checks if user has active MFA. If active, triggers **`«extend»` UC-02b: Validate MFA / TOTP Challenge**.<br>**4. System Response (Zero-Trust Branch):** If user holds an administrative role (`admin` or `super_admin`) without MFA enabled, system halts standard token generation, issues a scoped `mfa:enroll_only` token (10-min TTL), logs `MFA_ENROLLMENT_TOKEN_ISSUED`, and returns `{ mfa_setup_required: true, mfa_enrollment_token }` directing actor to **UC-05 (Flow 5a)**.<br>**5. System Response (Standard Path):** System generates JWT Access Token and Refresh Token with new family ID.<br>**6. System Response:** System stores hashed refresh token and family record in PostgreSQL DB.<br>**7. System Response:** System returns authenticated session payload to user. |
| **Alternative / Exceptional Flows** | **Flow 2a (Invalid Credentials):** If password verification fails, system increments failed attempt counter, records `AUTH_FAILURE` audit log, and returns `HTTP 401 Unauthorized`.<br>**Flow 2b (Account Locked):** If failed attempts exceed threshold (5 attempts), system locks account for 15 minutes and returns `HTTP 423 Locked`.<br>**Flow 2c (Zero-Trust Administrative Promotion Intercept):** If credentials are valid but the user holds an administrative tier without confirmed MFA, the system withholds standard access and refresh tokens, returning `HTTP 200 OK` with `{ mfa_setup_required: true, mfa_enrollment_token }` (`scope: 'mfa:enroll_only'`). The actor is routed to `/mfa-setup` and must complete **UC-05 (Flow 5a)** before elevated tokens are issued. |
...
| **Post-Conditions** | 1. Encrypted secret stored in PostgreSQL; `mfa_enabled` set to `true`.<br>2. For enrollment token flows: TOTP code is verified first, user row locked `FOR UPDATE`, and enrollment token `jti` claimed atomically in Redis (`SET NX`) with compensating Lua release; elevated session tokens are issued only upon confirmed verification. |
...
| **Zero-Trust Post-Promotion Flow** | **Flow 5a (Mandatory Administrative Enrollment):**<br>**1.** Promoted user authenticates at `/login` and receives scoped `mfa:enroll_only` token (10m TTL).<br>**2.** System routes user to `/mfa-setup` and initiates enrollment via `authenticateMfaEnrollment` middleware.<br>**3.** Promoted user scans QR code and submits 6-digit TOTP code to `/api/v1/mfa/verify`.<br>**4.** System verifies 6-digit TOTP code against decrypted secret within $\pm 1$ time step window (30s). If invalid, system returns `HTTP 400 Bad Request` without consuming the enrollment token, enabling user retries within token TTL.<br>**5.** Under user row lock `FOR UPDATE`, system verifies user role still mandates MFA (rejecting with `HTTP 401 AUTH_MFA_SETUP_NOT_REQUIRED` if demoted in the interim).<br>**6.** System atomically claims token in Redis via `SET mfa:enrollment:<jti> claimToken EX <TTL> NX` to prevent replay.<br>**7.** System commits `mfa_enabled = true`, issues fresh administrative Access & Refresh tokens, logs `MFA_ENABLED` audit event, and directs user to `/dashboard`. If database transaction fails, a compensating Lua script safely releases the Redis claim. |
...
| **Primary Actor** | Super Admin (Tier 3) |
| **Secondary Actor** | PostgreSQL DB (Transaction Client), Redis Cache |
| **Use Case Type** | Administrative Core Use Case |
| **Description** | Allows Super Administrators to search, inspect, activate/deactivate accounts, and govern the user role lifecycle with pessimistic row locking, atomic tier replacement, transaction-bound session revocation, and directional audit logging. |
| **Pre-Conditions** | Super Administrator possesses authenticated session with `user:read` and `role:update` permissions (`role:update` is granted exclusively to `super_admin` in the baseline RBAC matrix). |
| **Post-Conditions** | Target user account state/roles updated in PostgreSQL DB; all active refresh-token lineages revoked in PostgreSQL within the same transaction boundary (terminating persistent session continuation via RTR and bounding access token lifetime to remaining 15-minute TTL; reporting `sessionsRevoked: true`). |
| **Typical Course of Events (Normal Role Assignment)** | **1. Actor Action:** Super Administrator selects user and submits new role assignment.<br>**2. System Response:** System opens PostgreSQL transaction and acquires pessimistic lock: `SELECT id, email FROM users WHERE id = $1 FOR UPDATE`.<br>**3. System Response:** System atomically deletes conflicting system-tier roles (`super_admin`, `admin`, `user`) and inserts target role.<br>**4. System Response:** System determines privilege transition (Promotion or Demotion).<br>**5. System Response:** System executes `tokenService.revokeAllUserTokens(userId, client)` inside the same transaction boundary, revoking all active refresh tokens in PostgreSQL.<br>**6. System Response:** If demoting an elevated administrator to standard `user`, system purges pending unconfirmed MFA secrets.<br>**7. System Response:** System writes directional audit entries: `ROLE_ASSIGNED` and either `ROLE_PROMOTION_SESSION_REVOKED` (for promotion) or `TOKEN_REVOKED` (for demotion).<br>**8. System Response:** System commits transaction (`COMMIT`) and returns `{ assigned: true, sessionsRevoked: true, roleName }`. |
